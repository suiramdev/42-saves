...
/*   Updated: 2022/09/25 21:38:10 by mnouchet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	is_valid_row(int **grid, int pos, int *clues)
{
	int row;
	int i;
	int count[2];
	...
	while (i < GRID_SIZE)
	{
		if (grid[row][GRID_SIZE - 1 - i] > max)
		{
			max = grid[row][GRID_SIZE - 1 - i];
			count[1]++;
		}
		i++;
	}
	return (count[0] == clues[2 * GRID_SIZE + row]
		&& count[1] == clues[3 * GRID_SIZE + row]);
}

int	is_valid_col(int **grid, int pos, int *clues)
{
	int col;
	int i;
	int count[2];
	...
	while (i < GRID_SIZE)
	{
		if (grid[GRID_SIZE - 1 - i][col] > max)
		{
			max = grid[GRID_SIZE - 1 - i][col];
			count[1]++;
		}
		i++;
	}
	return (count[0] == clues[col]
		&& count[1] == clues[GRID_SIZE + col]);
}

int	can_place(int nb, int **grid, int pos, int *clues)
{
	return (is_number_available(nb, grid, pos)
			&& (is_valid_row(grid, pos, clues) || !is_row_filled(grid, pos))
			&& (is_valid_col(grid, pos, clues) || !is_col_filled(grid, pos)));
}
